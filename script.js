...
document.addEventListener('DOMContentLoaded', () => {
    ...
    navToggle.addEventListener('click', (e) => {
        ...
        navToggle.innerHTML = !visibility ? '<i class="fas fa-times"></i>' : '<i class="fas fa-bars"></i>';
    });

    // Close mobile menu when clicking outside
    document.addEventListener('click', (e) => {
        if (window.innerWidth <= 768) {
            const isNavButton = e.target.closest('.mobile-nav-toggle');
            const isNavMenu = e.target.closest('.primary-navigation');
            
            if (!isNavButton && !isNavMenu && primaryNav.getAttribute('data-visible') === "true") {
